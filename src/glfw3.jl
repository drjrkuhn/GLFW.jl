...
Base.cconvert(::Type{Window}, handle::WindowHandle) = ccall( (:glfwGetWindowUserPointer, libglfw), Ref{Window}, (WindowHandle,), handle)
...
Init() = Bool(ccall( (:glfwInit, libglfw), Cint, ())) || error("initialization failed")
Terminate() = ccall( (:glfwTerminate, libglfw), Void, ())
...
function GetMonitorPos(monitor::Monitor)
	...
	ccall( (:glfwGetMonitorPos, libglfw), Void, (Monitor, Ref{Cint}, Ref{Cint}), monitor, xpos, ypos)
	(xpos[], ypos[])
end

function GetMonitorPhysicalSize(monitor::Monitor)
	width, height = Ref{Cint}(), Ref{Cint}()
	ccall( (:glfwGetMonitorPhysicalSize, libglfw), Void, (Monitor, Ref{Cint}, Ref{Cint}), monitor, width, height)
	(width[], height[])
end
...
SetGamma(monitor::Monitor, gamma::Real) = ccall( (:glfwSetGamma, libglfw), Void, (Monitor, Cfloat), monitor, gamma)

# Window handling
DefaultWindowHints() = ccall( (:glfwDefaultWindowHints, libglfw), Void, ())
WindowHint(target::Integer, hint::Integer) = ccall( (:glfwWindowHint, libglfw), Void, (Cint, Cint), target, hint)
...
function CreateWindow(width::Integer, height::Integer, title::AbstractString, monitor::Monitor=Monitor(C_NULL), share::Window=Window(C_NULL))
	handle = ccall( (:glfwCreateWindow, libglfw), WindowHandle, (Cint, Cint, Cstring, Monitor, WindowHandle), width, height, title, monitor, share)
	window = Window(handle)
	ccall( (:glfwSetWindowUserPointer, libglfw), Void, (WindowHandle, Ref{Window}), window, window)
	window
end

DestroyWindow(window::Window) = (yield(); ccall( (:glfwDestroyWindow, libglfw), Void, (WindowHandle,), window))
WindowShouldClose(window::Window) = ccall( (:glfwWindowShouldClose, libglfw), Cint, (WindowHandle,), window) != 0
SetWindowShouldClose(window::Window, value::Bool) = ccall( (:glfwSetWindowShouldClose, libglfw), Void, (WindowHandle, Cint), window, value)
SetWindowTitle(window::Window, title::AbstractString) = ccall( (:glfwSetWindowTitle, libglfw), Void, (WindowHandle, Cstring), window, title)

function GetWindowPos(window::Window)
	xpos, ypos = Ref{Cint}(), Ref{Cint}()
	ccall( (:glfwGetWindowPos, libglfw), Void, (WindowHandle, Ref{Cint}, Ref{Cint}), window, xpos, ypos)
	(xpos[], ypos[])
end

SetWindowPos(window::Window, xpos::Integer, ypos::Integer) = ccall( (:glfwSetWindowPos, libglfw), Void, (WindowHandle, Cint, Cint), window, xpos, ypos)

function GetWindowSize(window::Window)
	width, height = Ref{Cint}(), Ref{Cint}()
	ccall( (:glfwGetWindowSize, libglfw), Void, (WindowHandle, Ref{Cint}, Ref{Cint}), window, width, height)
	(width[], height[])
end

SetWindowSize(window::Window, width::Integer, height::Integer) = ccall( (:glfwSetWindowSize, libglfw), Void, (WindowHandle, Cint, Cint), window, width, height)

function GetFramebufferSize(window::Window)
	width, height = Ref{Cint}(), Ref{Cint}()
	ccall( (:glfwGetFramebufferSize, libglfw), Void, (WindowHandle, Ref{Cint}, Ref{Cint}), window, width, height)
	(width[], height[])
end

function GetWindowFrameSize(window::Window)
	left, top, right, bottom = Ref{Cint}(), Ref{Cint}(), Ref{Cint}(), Ref{Cint}()
	ccall( (:glfwGetWindowFrameSize, libglfw), Void, (WindowHandle, Ref{Cint}, Ref{Cint}, Ref{Cint}, Ref{Cint}), window, left, top, right, bottom)
	(left[], top[], right[], bottom[])
end

IconifyWindow(window::Window) = ccall( (:glfwIconifyWindow, libglfw), Void, (WindowHandle,), window)
RestoreWindow(window::Window) = ccall( (:glfwRestoreWindow, libglfw), Void, (WindowHandle,), window)
ShowWindow(window::Window) = ccall( (:glfwShowWindow, libglfw), Void, (WindowHandle,), window)
HideWindow(window::Window) = ccall( (:glfwHideWindow, libglfw), Void, (WindowHandle,), window)
GetWindowMonitor(window::Window) = ccall( (:glfwGetWindowMonitor, libglfw), Monitor, (WindowHandle,), window)
GetWindowAttrib(window::Window, attrib::Integer) = ccall( (:glfwGetWindowAttrib, libglfw), Cint, (WindowHandle, Cint), window, attrib)
...
PollEvents() = ccall( (:glfwPollEvents, libglfw), Void, ())
WaitEvents() = ccall( (:glfwWaitEvents, libglfw), Void, ())
PostEmptyEvent() = ccall( (:glfwPostEmptyEvent, libglfw), Void, ())

# Input handling
function GetInputMode(window::Window, mode::Integer)
	value = ccall( (:glfwGetInputMode, libglfw), Cint, (WindowHandle, Cint), window, mode)
...
SetInputMode(window::Window, mode::Integer, value::Integer) = ccall( (:glfwSetInputMode, libglfw), Void, (WindowHandle, Cint, Cint), window, mode, value)
GetKey(window::Window, key::Integer) = Bool(ccall( (:glfwGetKey, libglfw), Cint, (WindowHandle, Cint), window, key))
GetMouseButton(window::Window, button::Integer) = Bool(ccall( (:glfwGetMouseButton, libglfw), Cint, (WindowHandle, Cint), window, button))

function GetCursorPos(window::Window)
	xpos, ypos = Ref{Cdouble}(), Ref{Cdouble}()
	ccall( (:glfwGetCursorPos, libglfw), Void, (WindowHandle, Ref{Cdouble}, Ref{Cdouble}), window, xpos, ypos)
	(xpos[], ypos[])
end

SetCursorPos(window::Window, xpos::Real, ypos::Real) = ccall( (:glfwSetCursorPos, libglfw), Void, (WindowHandle, Cdouble, Cdouble), window, xpos, ypos)
CreateStandardCursor(shape::Integer) = ccall( (:glfwCreateStandardCursor, libglfw), Cursor, (Cint,), shape)
DestroyCursor(cursor::Cursor) = ccall( (:glfwDestroyCursor, libglfw), Void, (Cursor,), cursor)
SetCursor(window::Window, cursor::Cursor) = ccall( (:glfwSetCursor, libglfw), Void, (WindowHandle, Cursor), window, cursor)
...
JoystickPresent(joy::Integer) = Bool(ccall( (:glfwJoystickPresent, libglfw), Cint, (Cint,), joy))
...
GetJoystickName(joy::Integer) = unsafe_string(ccall( (:glfwGetJoystickName, libglfw), Cstring, (Cint,), joy))

# Context handling
MakeContextCurrent(window::Window) = ccall( (:glfwMakeContextCurrent, libglfw), Void, (WindowHandle,), window)
GetCurrentContext() = Base.cconvert(Window, ccall( (:glfwGetCurrentContext, libglfw), WindowHandle, ()))
SwapBuffers(window::Window) = ccall( (:glfwSwapBuffers, libglfw), Void, (WindowHandle,), window)
SwapInterval(interval::Integer) = ccall( (:glfwSwapInterval, libglfw), Void, (Cint,), interval)
ExtensionSupported(extension::AbstractString) = Bool(ccall( (:glfwExtensionSupported, libglfw), Cint, (Cstring,), extension))
GetProcAddress(procname::AbstractString) = ccall((:glfwGetProcAddress, libglfw), Ptr{Void}, (Cstring,), procname)
